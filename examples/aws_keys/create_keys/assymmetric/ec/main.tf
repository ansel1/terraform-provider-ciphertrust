...
locals {
  ...
  ec_key_alias_ex1 = "TestAws-ex1-${lower(random_id.random.hex)}"
  ec_key_alias_ex2 = "TestAws-ex2-${lower(random_id.random.hex)}"
...
#
resource "ciphertrust_aws_key" "ec_key_ex1" {
  alias                              = [local.ec_key_alias_ex1, local.extra_alias_2, local.extra_alias_3]
  auto_rotate                        = false
  bypass_policy_lockout_safety_check = true
  description                        = "terraform create ec key example"
  enable_key                         = true
...
output "ec_key_ex1" {
  value = ciphertrust_aws_key.ec_key_ex1
}

#
# Minumum use of input parameters for an EC key
#
resource "ciphertrust_aws_key" "ec_key_ex2" {
  alias                    = [local.ec_key_alias_ex2]
  kms                      = ciphertrust_aws_kms.kms.id
  customer_master_key_spec = "ECC_SECG_P256K1"
  region                   = ciphertrust_aws_kms.kms.regions[0]
}
output "ec_key_ex2" {
  value = ciphertrust_aws_key.ec_key_ex2
}